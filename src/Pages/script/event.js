...
async function jsonFromPHP() {
    try {
        const response = await fetch('../Database/phpToJS.php')
            .then(function (response) {
                return response.json();
            }).then(function (data) {
                eventsFromPhp = data;
                rows = eventsFromPhp.length;
                console.log(rows + "# of rows fetched from database");
                if (rows == 0) {
                    emptyEvents();
                    return;
                }
...
function createNewPost(e) {
    ...
    for (i = firstRowInTab - 1; i < lastRowInTab; i++) {
        ...
        var number_req = obj[6];
        var number_curr = obj[7];
        var start_date = obj[8];
        var start_time = obj[10];
        var end_time = obj[11];
        var point = calcHours(start_time, end_time) * 2;
        ...
        // if(!imageExists(img)){
        //     img = "../image/icon-organization.png";
        // }
...
function addPostInHTML(img, org, title, img, city, address, google_map, number_req, number_curr,
    ...
    label6.innerText = start_time + " - " + end_time;
...
function detailsEvent(c) {
    ...
    var number_req = objj[6];
    var number_curr = objj[7];
    var start_date = objj[8];
    var end_date = objj[9];
    var start_time = objj[10];
    var end_time = objj[11];
    var instructor_name = objj[12];
    var instructor_phone = objj[13];
    var instructor_email = objj[14];
    var status = objj[15];
    var description = objj[16];

    var point = calcHours(start_time, end_time);
    ...
    detailsEvent_create(img, org, title, img, city, address, google_map, number_req, number_curr,
        start_date, end_date, start_time, end_time, instructor_name, instructor_phone, instructor_email, status, point, description);
}
...
function detailsEvent_create(img, org, title, img, city, address, google_map, number_req, number_curr,
    ...
    labelT.innerText = "اسم المنشأة: " + org;
    ...
    labelH.innerText = "الساعات: " + calcHours(start_time, end_time);
    ...
    label8.innerText = "بريد الاكتروني المشرف: " + instructor_email;
    ...
    console.log(i + "# Event details window created (will be deleted as soon it closed)");
}
...
function searchEvent(e) {
    ...
    // Title Search
    for (i = 0; i < eventsFromPhp.length - 1; i++) {
        var obj = eventsFromPhp[i];
...
function fillNewEvents(arr) {
    ...
    for (i = firstRowInTab - 1; i < lastRowInTab; i++) {
        ...
        // if(!imageExists(img)){
            ...
            console.log(i + "# matched the search filter");
...
//     '{"start_time": "17:40"},' +
//     '{"end_time": "19:40"},' +
//     '{"instructor_name": "اسم المشرف"},' +
//     '{"instructor_phone": "رقم المشرف"},' +
//     '{"instructor_email": "بريد الاكتروني المشرف" }]}';

// const obj = JSON.parse(text);


// let eventsFromPhp = [...Array(rows)].map(() => ({
//     event: obj
// }));