...
<body>
  ...
  $my_query = mysqli_query($conn, "SELECT password FROM organization WHERE name=   '{$orgname}' ");
  ...
  if(isset($_POST['save'])) {
    ...
    if ($new == $re_new){
        ...
        $update = "update organization set password= '$new_pass' WHERE name='" . $orgname . "'";
        $conn->query($update);

        if ($conn->query($update) === TRUE) {
            echo "password updated successfully";
            echo "<script>alert('$suceess');</script>";
            echo "<script>window.location ='OrgPass.php';</script>";
